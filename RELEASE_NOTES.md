...
Issue titles now follow the same convention as every other app — `MovieTickets v1.11.x — <headline>`,
labelled `pass` — instead of the `pass: <headline>` this app had invented.

### Reporting is a library now

The eight files under `com.gios.lightpass.report` are gone. They are
